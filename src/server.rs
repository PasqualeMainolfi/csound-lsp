    ...
    async fn did_change(&self, params: DidChangeTextDocumentParams) {
        ...
        if let Some(doc) = d.get_mut(&uri) {
            ...
            if !resolve_udos::add_included_udos_to_cs_references(&doc.cached_included_udo_files, &mut jr) {
                self.client.log_message(MessageType::WARNING, format!("[WARNING] Included User-Defined opcodes: external opcodes cache corrupted! cached: {:?}", doc.cached_included_udo_files)).await;
            }
